...
using haxe.linq.Iterator;
using haxe.linq.Function;
...
class Manipulation 
{
	...
	public static function selectMany<T, U, V>(subject:Iterable<T>, collection:Iterable<U>, selector: T -> U -> V):Iterable<V> {
		return new haxe.linq.ConcatIterable(subject.select(function(first) return collection.select(function(second) return selector(first, second))));
		return new SelectManyIterable(subject, collection, selector);
	}
